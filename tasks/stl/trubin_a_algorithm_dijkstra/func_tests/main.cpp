...
TEST(trubin_a_algorithm_dijkstra_stl, random_large_graph) {
...
TEST(trubin_a_algorithm_dijkstra_stl, complex_cycle_graph) {
  std::vector<int> graph = {1, 1, -1, 2, 1, -1, 0, 1, 3, 1, -1, -1};
  RunDijkstraTest(graph, 0, 4, {0, 1, 2, 3});
}

TEST(trubin_a_algorithm_dijkstra_stl, cycle_with_shortcut_graph) {
  std::vector<int> graph = {1, 1, 2, 5, -1, 2, 1, -1, 0, 1, -1};
  RunDijkstraTest(graph, 0, 3, {0, 1, 2});
}
