...
class zaitsev_a_labeling_test_stl : public ::testing::TestWithParam<std::string> {
 protected:
#ifndef _WIN32
  ...
  static TestData PrepareImages(const std::string& filename) {
    cv::Mat img_raw = cv::imread(ppc::util::GetAbsolutePath("stl/zaitsev_a_bw_labeling/data/" + filename));
...
TEST_F(zaitsev_a_labeling_test_stl, validation_fails_on_incorrect_input) {
  const int width = 16;
...
#ifndef _WIN32
TEST_P(zaitsev_a_labeling_test_stl, returns_correct_label_map) {
...
// clang-format off
INSTANTIATE_TEST_SUITE_P(zaitsev_a_labeling_test_stl, zaitsev_a_labeling_test_stl, ::testing::Values(
    "help.jpg",
    "kittens.jpg",
    "rombs.jpg",
    "silly.jpg",
