...
TEST(belov_a_radix_batcher_mergesort_stl, test_pipeline_run) {
  // Create data
  int n = 4194304;
  ...
  // Create Task
  auto tesk_task_tbb =
      std::make_shared<belov_a_radix_batcher_mergesort_stl::RadixBatcherMergesortParallel>(task_data_stl);
...
TEST(belov_a_radix_batcher_mergesort_stl, test_task_run) {
  // Create data
  int n = 4194304;
  std::vector<Bigint> arr = GenerateMixedValuesArray(n);
