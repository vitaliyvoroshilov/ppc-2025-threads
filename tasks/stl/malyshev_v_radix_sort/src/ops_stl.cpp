...
void malyshev_v_radix_sort_stl::RadixDoubleSort(std::vector<double>& data, size_t first, size_t last) {
...
void malyshev_v_radix_sort_stl::Merge(std::vector<double>& target_array, size_t merge_start, size_t mid_point,
                                      size_t merge_end) {
  std::vector<double> merged;
  merged.reserve(merge_end - merge_start);

  auto start_iter = target_array.begin() + static_cast<std::ptrdiff_t>(merge_start);
  auto mid_iter = target_array.begin() + static_cast<std::ptrdiff_t>(mid_point);
  auto end_iter = target_array.begin() + static_cast<std::ptrdiff_t>(merge_end);

  std::merge(start_iter, mid_iter, mid_iter, end_iter, std::back_inserter(merged));
  std::ranges::copy(merged, start_iter);
}

bool malyshev_v_radix_sort_stl::TestTaskSTL::RunImpl() {
      ...
      merge_threads.emplace_back([&, group_range_start, group_range_end, current_merge_size, merge_group_size] {
        for (size_t group_index = group_range_start; group_index < group_range_end; ++group_index) {
          ...
          if (merge_mid < merge_end) {
            Merge(input_data_, merge_start, merge_mid, merge_end);
          }
        }
      });
    }
...
bool malyshev_v_radix_sort_stl::TestTaskSTL::PreProcessingImpl() {
...
bool malyshev_v_radix_sort_stl::TestTaskSTL::ValidationImpl() {
  return task_data->inputs_count[0] > 0 && task_data->outputs_count[0] == task_data->inputs_count[0];
}

bool malyshev_v_radix_sort_stl::TestTaskSTL::PostProcessingImpl() {
