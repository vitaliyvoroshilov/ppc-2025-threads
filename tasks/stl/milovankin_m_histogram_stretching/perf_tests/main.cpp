...
TEST(milovankin_m_histogram_stretching_stl, test_pipeline_run) {
  std::vector<uint8_t> data_in(123456789, 123);
  std::vector<uint8_t> data_out(data_in.size());
  data_in.front() = 5;
  data_in.back() = 155;

  auto task = std::make_shared<milovankin_m_histogram_stretching_stl::TestTaskSTL>(CreateTask(data_in, data_out));
...
TEST(milovankin_m_histogram_stretching_stl, test_task_run) {
  std::vector<uint8_t> data_in(123456789, 123);
  std::vector<uint8_t> data_out(data_in.size());
  data_in.front() = 5;
  data_in.back() = 155;
