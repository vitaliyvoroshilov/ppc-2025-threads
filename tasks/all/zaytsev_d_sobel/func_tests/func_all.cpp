...
TEST(zaytsev_d_sobel_all, test_validation_fail_count_in_out) {
  ...
  zaytsev_d_sobel_all::TestTaskALL task(task_data);
  ASSERT_EQ(task.Validation(), false);
}

TEST(zaytsev_d_sobel_all, test_validation_fail_small_image) {
  boost::mpi::communicator world;
...
TEST(zaytsev_d_sobel_all, test_validation_wrong_size) {
  boost::mpi::communicator world;
  std::vector<int> in(10, 0);
  std::vector<int> out(10, 0);
...
TEST(zaytsev_d_sobel_all, SobelEdgeDetection_5x5) {
  boost::mpi::communicator world;
  ...
  zaytsev_d_sobel_all::TestTaskALL sobel_task(task_data);
...
TEST(zaytsev_d_sobel_all, Sobel_12x12) {
  boost::mpi::communicator world;
...
TEST(zaytsev_d_sobel_all, SobelEdgeDetection_UniformImage) {
  boost::mpi::communicator world;
  constexpr size_t kSize = 5;
...
#ifndef _WIN32
TEST(zaytsev_d_sobel_all, SobelEdgeDetection_OpenCVImage) {
  boost::mpi::communicator world;
  cv::Mat input_img =
      cv::imread(ppc::util::GetAbsolutePath("all/zaytsev_d_sobel/data/inwhite.png"), cv::IMREAD_GRAYSCALE);
  cv::Mat expected_img =
      cv::imread(ppc::util::GetAbsolutePath("all/zaytsev_d_sobel/data/outputwhite.png"), cv::IMREAD_GRAYSCALE);
  ...
  if (world.rank() == 0) {
    input = MatToVector(input_img);
    expected = MatToVector(expected_img);
    output.resize(input.size(), 0);
    size = {input_img.cols, input_img.rows};
  }
...
TEST(zaytsev_d_sobel_all, Sobel_Circle) {
  boost::mpi::communicator world;
...
TEST(zaytsev_d_sobel_all, RectangularImage_18x20) {
  boost::mpi::communicator world;
