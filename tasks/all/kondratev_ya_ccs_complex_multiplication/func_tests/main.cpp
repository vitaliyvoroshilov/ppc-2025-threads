...
kondratev_ya_ccs_complex_multiplication_all::CCSMatrix ConvertToCCS(
    const std::vector<std::complex<double>> &dense_matrix, std::pair<int, int> sizes) {
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix sparse(sizes);
  ...
  for (int col = 0; col < sparse.cols; col++) {
    ...
    for (int row = 0; row < sparse.rows; row++) {
      ...
      if (kondratev_ya_ccs_complex_multiplication_all::IsZero(dense_matrix[idx])) {
        continue;
      }
...
bool IsComplexVectorEqual(const std::vector<std::complex<double>> &a, const std::vector<std::complex<double>> &b) {
  return std::ranges::equal(
      a, b, [](const auto &x, const auto &y) { return kondratev_ya_ccs_complex_multiplication_all::IsEqual(x, y); });
}
...
void CCSExpectEqual(const kondratev_ya_ccs_complex_multiplication_all::CCSMatrix &a,
                    const kondratev_ya_ccs_complex_multiplication_all::CCSMatrix &b) {
...
struct Matrices {
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix &in1;
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix &in2;
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix &out;
};
...
void RunTest(Matrices matrices) {
  ...
  if (world.rank() == 0) {
    task_data_all->inputs.emplace_back(reinterpret_cast<uint8_t *>(&matrices.in1));
    task_data_all->inputs.emplace_back(reinterpret_cast<uint8_t *>(&matrices.in2));
    task_data_all->inputs_count.emplace_back(2);

    task_data_all->outputs.emplace_back(reinterpret_cast<uint8_t *>(&matrices.out));
    task_data_all->outputs_count.emplace_back(1);
  }

  kondratev_ya_ccs_complex_multiplication_all::TestTaskALL test_task_sequential(task_data_all);
...
TEST(kondratev_ya_ccs_complex_multiplication_all, matrix_convert_to_ccs) {
  // clang-format off
...
TEST(kondratev_ya_ccs_complex_multiplication_all, generate_random_sparse_matrix) {
  int rows = 5;
  int cols = 5;
  double density = 0.2;
  ...
  for (size_t i = 0; i < sparse_matrix.size(); i++) {
    if (!kondratev_ya_ccs_complex_multiplication_all::IsZero(sparse_matrix[i])) {
...
TEST(kondratev_ya_ccs_complex_multiplication_all, test_classic_matrix_multiplication) {
  // clang-format off
...
TEST(kondratev_ya_ccs_complex_multiplication_all, test_empty_matrices) {
  static boost::mpi::communicator world;
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix a;
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix b;
  ...
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix c;
  task_data->outputs.emplace_back(reinterpret_cast<uint8_t *>(&c));
  task_data->outputs_count.emplace_back(1);

  kondratev_ya_ccs_complex_multiplication_all::TestTaskALL task(task_data);
...
TEST(kondratev_ya_ccs_complex_multiplication_all, test_single_element) {
  auto task_data = std::make_shared<ppc::core::TaskData>();
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix a({1, 1});
  a.values = {{2.0, 1.0}};
  a.row_index = {0};
  a.col_ptrs = {0, 1};

  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix b({1, 1});
  b.values = {{3.0, -1.0}};
  b.row_index = {0};
  b.col_ptrs = {0, 1};

  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix expected({1, 1});
  expected.values = {{7.0, 1.0}};
  expected.row_index = {0};
  expected.col_ptrs = {0, 1};

  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix c({2, 2});
  RunTest({.in1 = a, .in2 = b, .out = c});
  CCSExpectEqual(c, expected);
}

TEST(kondratev_ya_ccs_complex_multiplication_all, test_small_ccs_multiplication) {
  // Матрица A (2x3):
  // [1+0i  0    2+1i]
  // [0     3-1i 0   ]
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix a({2, 3});
  ...
  // [0     3-1i]
  // [2+1i  0   ]
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix b({3, 2});
  ...
  // [4+4i  2+0i]
  // [0     8-6i]
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix expected({2, 2});
...
TEST(kondratev_ya_ccs_complex_multiplication_all, random_matrix_multiplication) {
  int rows = 5;
  int cols = 5;
  double density = 0.2;
  ...
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix ccs_c({rows, cols});
...
TEST(kondratev_ya_ccs_complex_multiplication_all, test_incompatible_matrix_sizes) {
  static boost::mpi::communicator world;
  ...
  kondratev_ya_ccs_complex_multiplication_all::CCSMatrix c({3, 4});
