...
void TestRun(ppc::core::TaskDataPtr& task_data) {
  auto task = std::make_shared<malyshev_a_increase_contrast_by_histogram_all::TestTaskALL>(task_data);
...
TEST(malyshev_a_increase_contrast_by_histogram_all, all_pixels_are_equal) {
...
TEST(malyshev_a_increase_contrast_by_histogram_all, black_and_white) {
  constexpr size_t kSize = 10;
...
TEST(malyshev_a_increase_contrast_by_histogram_all, low_contrast) {
...
#ifndef _WIN32
TEST(malyshev_a_increase_contrast_by_histogram_all, pic_from_file) {
  std::vector<uint8_t> input;
  cv::Mat img;
  cv::Mat reference;
  if (GetRank() == 0) {
    img = cv::imread(ppc::util::GetAbsolutePath("all/malyshev_a_increase_contrast_by_histogram/data/input.jpg"),
                     cv::IMREAD_GRAYSCALE);
    reference =
        cv::imread(ppc::util::GetAbsolutePath("all/malyshev_a_increase_contrast_by_histogram/data/reference.jpg"),
                   cv::IMREAD_GRAYSCALE);

    input.assign(img.data, img.data + img.total());
  }
...
TEST(malyshev_a_increase_contrast_by_histogram_all, invalid_input) {
  std::vector<uint8_t> input = {1, 2, 3};
  std::vector<uint8_t> output(2);

  auto task_data = TestPrepare(input, output);

  malyshev_a_increase_contrast_by_histogram_all::TestTaskALL task(task_data);
  if (GetRank() == 0) {
    EXPECT_FALSE(task.Validation());
  }
}

TEST(malyshev_a_increase_contrast_by_histogram_all, empty_input) {
  std::vector<uint8_t> input;
...
TEST(malyshev_a_increase_contrast_by_histogram_all, single_pixel) {
...
TEST(malyshev_a_increase_contrast_by_histogram_all, preserve_order) {
