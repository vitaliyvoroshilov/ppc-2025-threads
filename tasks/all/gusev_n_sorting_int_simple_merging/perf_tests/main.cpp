...
TEST(gusev_n_sorting_int_simple_merging_all, test_pipeline_run) {
  constexpr int kCount = 5000000;
  auto test_data = GenerateTestData(kCount);

  auto test_task =
      std::make_shared<gusev_n_sorting_int_simple_merging_all::SortingIntSimpleMergingALL>(test_data.task_data);
...
TEST(gusev_n_sorting_int_simple_merging_all, test_task_run) {
  constexpr int kCount = 5000000;
  auto test_data = GenerateTestData(kCount);
