...
TEST(tyurin_m_matmul_crs_complex_all, test_regular_matrix_mult_inv) {
...
TEST(tyurin_m_matmul_crs_complex_all, test_crs_random_inv) {
  TestMatrixCRS({.rows = 3, .cols = 3, .data = {1, 0, 0, 1, -1, 0, 1, 0, 1}},
                {.rows = 3, .cols = 3, .data = {1, 0, 0, 1, -1, 0, -1, 0, 1}});
}
TEST(tyurin_m_matmul_crs_complex_all, validation_failure) {
  int rank{};
  MPI_Comm_rank(MPI_COMM_WORLD, &rank);
  if (rank != 0) {
    return;
