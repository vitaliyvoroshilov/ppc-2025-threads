...
Component voroshilov_v_convex_hull_components_omp::DepthComponentSearchInArea(Pixel start_pixel, Image& image,
                                                                              int area_top, std::vector<uint8_t>& thread_visited,
  ...
  while (!stack.empty()) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (image.pixels[next_index].value != 1 || thread_visited[next_index] || local_visited[next_index]) {
        continue;
      }
      if (next_y < area_top) {
        for (int index : visited_indexes) {
          local_visited[index] = 0;
        }
        return {};
      }
      local_visited[next_index] = 1;
      visited_indexes.push_back(next_index);
      stack.push_back(next_index);
    }
  }

  for (int index : visited_indexes) {
    local_visited[index] = 1;
    thread_visited[index] = 1;
  }

  return Component(std::move(component));
}

std::vector<Component> voroshilov_v_convex_hull_components_omp::FindComponentsInArea(Image& image, int area_top, int area_bottom) {
  int height = image.height;
  int width = image.width;
  std::vector<uint8_t> thread_visited(height * width, 0);
  std::vector<uint8_t> local_visited(height * width, 0);
  ...
  for (int y = area_top; y < area_bottom; y++) {
    for (int x = 0; x < width; x++) {
      int index = (y * width) + x;
      if (image.GetPixel(y, x) == 0 || thread_visited[index]) {
        continue;
      }
      if (y >= area_top && image.GetPixel(y - 1, x).value == 1) {
        continue;
      }
      Component component = DepthComponentSearchInArea(image.GetPixel(y, x), image, area_top, thread_visited, local_visited);
      if (!component.empty()) {
...
std::vector<std::vector<Component>> voroshilov_v_convex_hull_components_omp::FindComponentsOMP(Image& image) {
  ...
  std::cout << "[OMP | local_images: " << duration.count() << " ms]" << std::endl;
...
std::vector<Hull> voroshilov_v_convex_hull_components_omp::QuickHullAllOMP(std::vector<std::vector<Component>>& threads_components) {
  if (threads_components.empty()) {
    return {};
  }

  int num_threads = omp_get_max_threads();
  std::vector<std::vector<Hull>> threads_hulls(num_threads);

  auto start = std::chrono::high_resolution_clock::now();

#pragma omp parallel for schedule(dynamic)
  for (int t = 0; t < num_threads; t++) {
    std::vector<Component>& local_components = threads_components[t];
    for (Component& comp : local_components) {
      Hull hull = QuickHull(comp);
      threads_hulls[t].push_back(hull);
    }
  }

  auto end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double, std::milli> duration = end - start;
  std::cout << "[OMP | hulls: " << duration.count() << " ms]" << std::endl;
  start = std::chrono::high_resolution_clock::now();

  std::vector<Hull> hulls;
  size_t total_num_hulls = 0;
  for (std::vector<Hull>& local_hulls : threads_hulls) {
    total_num_hulls += local_hulls.size();
  }
  hulls.reserve(total_num_hulls);
  for (std::vector<Hull>& local_hulls : threads_hulls) {
    hulls.insert(hulls.end(), local_hulls.begin(), local_hulls.end());
  }
